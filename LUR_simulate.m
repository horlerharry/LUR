...
% 
    ...
    PU_AVG_SE = zeros(1,P); SU_AVG_SE = zeros(1,S);
    ...
    for u = 1:U
        ...
        secondary_c = abs((randn(S,N*P)+1i*randn(S,N*P))/sqrt(2));
        ...
        PU_AVG = zeros(1,P); SU_AVG = zeros(1,S);
        ...
        for pu = 1:P
            ...
            if(isempty(power))
                %i.e. No cooperation
                PU_AVG(pu) = PU_direct(pu);
            else
                relay = reshape(SU_PU_channels(pu,cda_su,:),N,1);
                channel_second = reshape(SU_channels(cda_su,pu,:),N,1);
                [SU_AVG(cda_su),PU_AVG(pu)] = C_NOMA(channel_second,...
                    PU_channels(:,pu),relay,power,p);
            end
        ...
        PU_AVG_SE = PU_AVG_SE + PU_AVG;
        SU_AVG_SE = SU_AVG_SE + SU_AVG;
    ...
    %Final manipulation
    outputs = cellfun(@(x) x./N, outputs,'UniformOutput',0);
end